...
<header class="no-content-header">
	<?php if ( $image ) : ?>
	<div class="no-content-cover" title="<?= esc_attr( $image['credit'] ) ?>" style="<?= $image ? sprintf( 'background-image: url(\'%s?%s\')', esc_url( $image['url'] ), esc_attr( $image['version'] ) ): '' ?>"  uk-parallax="bgy:-300"></div>
	<?php endif; ?>
	<div class="no-content-box">
		<div class="entry-container">
			...
			<p class="no-content-lead">
				<?= wp_kses_post( __( 'Please try <a href="#no-content">search</a> or other options.', 'kyom' ) ) ?>
			</p>
		</div>
	</div>
</header>
...
<div class="entry-container no-content" id="no-content">
	...
	<ul class="uk-list uk-list-large uk-list-bullet no-content-todo">
		...
		<li><?= wp_kses_post( $description ) ?></li>
		<?php endforeach; ?>
	</ul>
	<?php endif; ?>
