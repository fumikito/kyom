<header class="archive-header">
	...
	<?php
	// Cover image for terms.
	if ( is_category() || is_tag() || is_tax() ) :
		$url = kyom_term_image( get_queried_object(), 'full' );
		if ( $url ) :
			?>
			<div class="archive-header-cover" style="background-image: url( '<?php echo esc_url( $url ); ?>' )"  uk-parallax="bgy:-300">
			</div>
			<?php
		endif;
	endif;
	...
	<div class="uk-container">
		<h1 class="uk-heading-line uk-text-center archive-header-title"><span><?php echo wp_kses_post( kyom_archive_title() ); ?></span></h1>
		...
		<div class="archive-header-search">
			...
			echo wp_kses_post( sprintf(
				...
				sprintf( _n( '%s post', '%s posts', $found, 'kyom' ), number_format( $found ) ),
				max( 1, $wp_query->paged ),
				sprintf( _n( '%s page', '%s pages', $wp_query->max_num_pages, 'kyom' ), number_format( $wp_query->max_num_pages ) )
			) );
			?>
			<button class="uk-button uk-button-text uk-button-small" uk-toggle="target:#header-search"><span uk-icon="search"></span></button>
		</div>

		<form id="header-search" class="archive-header-form uk-form-horizontal" hidden aria-hidden="true" method="get" action="<?php echo home_url(); ?>">
			...
			<fieldset>
				...
				foreach ( kyom_searchable_taxonomies() as $taxonomy => $name ) :
					...
					?>
				<div class="uk-margin">
					<div class="uk-form-label"><?php echo esc_html( $taxonomy_object->label ); ?></div>
					<div class="uk-form-controls uk-form-controls-text">
						...
						<label class="uk-form-label-inline">
							<input class="uk-radio" type="radio" name="<?php echo esc_attr( $name ); ?>" value="<?php echo esc_attr( $value ); ?>" <?php checked( $selected ); ?>>
